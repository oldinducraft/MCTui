...
impl Jvm {
    pub async fn new(manifest: &Manifest, args: GameArgs) -> Jvm {
        assert_eq!(manifest.r#type, "release", "Only release types are supported");

        let jvm_args = Jvm::prepare_jvm_args(manifest).await;
    ...
    async fn prepare_jvm_args(manifest: &Manifest) -> Vec<String> {
    ...
    async fn classpath(manifest: &Manifest) -> String {
        ...
        resolved_paths
            .into_iter()
            .map(|path| dunce::canonicalize(path.unwrap()).unwrap().to_str().unwrap().to_string())
